...
This release introduces the Report Generator Plugin foundation.

Highlights:

- Added `reporting` plugin.
- Added `report-generator` agent.
- Added `report.generate` workflow metadata.
...
- Added `.dr-magu/reports/` persistence.
- Added `dr-magu report "Title"`.
- Added `dr-magu report-from-research`.

Example:

```bash
dr-magu report "Executive Summary" --summary "Initial report generated by Dr Magu."
dr-magu research "AI developer tools" --limit 5
dr-magu report-from-research
```


## v0.14.0 - HITL Approval Engine Foundation

This release introduces human-in-the-loop approval management.

Highlights:

- Added `approval` plugin.
- Added `approval-manager` agent.
- Added `approval.review` workflow metadata.
- Added `approval.request`, `approval.approve`, `approval.reject`, and `approval.list`.
- Added `.dr-magu/approvals/` persistence.
- Added approval guard helpers for high-risk actions.
- Prepared option-selection flows for future website generation and multi-agent orchestration.

Example:

```bash
dr-magu approval-request "Approve website architecture" --action website.generate --risk high
