...
def test_plugin_registry_discovers_workspace_plugins(tmp_path: Path) -> None:
    ...
    assert [plugin.id for plugin in plugins] == ["ai-operating-system", "approval", "autonomous-execution-loop", "autonomous-software-factory", "background-worker", "conversational-brain", "conversational-command-router", "execution-runtime", "llm-runtime", "mcp-research-runtime", "multi-agent-orchestrator", "platform-stabilization", "real-mcp-integrations", "reporting", "research", "scheduler", "self-healing-workflows", "software-dev", "software-development", "website-builder", "workflow-engine"]
...
def test_brain_context_includes_plugins(tmp_path: Path) -> None:
    ...
    assert snapshot.summary["plugin_count"] == 21
