...
def test_plugin_registry_discovers_workspace_plugins(tmp_path: Path) -> None:
    ...
    assert [plugin.id for plugin in plugins] == ["approval", "background-worker", "conversational-brain", "execution-runtime", "platform-stabilization", "reporting", "research", "scheduler", "software-dev", "software-development", "website-builder", "workflow-engine"]
...
def test_brain_context_includes_plugins(tmp_path: Path) -> None:
    ...
    assert snapshot.summary["plugin_count"] == 12
